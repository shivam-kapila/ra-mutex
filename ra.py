...
class Message(object):
    ...
    def __str__(self):
        return "Message: TYPE = " + ("None" if (self.type is None) else self.type) \
            + " FROM = " + ("None" if (self.type is None) else str(self.sender)) \
            + " CONTENT = " + ("None" if (self.content is None) else str(self.content))

    def prepare(self, msg_type=None, msg_from=None, msg_content=None):
        if (msg_type is None and msg_from is None and msg_content is None):
            pass
        elif (msg_type is not None and msg_from is not None and msg_content is not None):
...
class RA(object):
    ...
    def __check_awaiting_nodes(self):
        ...
        for node in list(self.awaiting_reply.keys()):
            if self.awaiting_reply.get(node):
    ...
    def __handle_request_message(self, args):
        ...
        defer_it = (self.requesting_cs and ((in_seq_num > self.seq_num) or (
            (in_seq_num == self.seq_num) and in_unique_name > self.info["UNIQUENAME"])))
        self.var_lock.release()
        if defer_it:
    ...
    def __handle_are_you_there_message(self, args):
        ...
        if (not self.reply_deffered.get(sender["UNIQUENAME"])):
            reply = Message()
            to_send = reply.prepare(Message.TYPE.REPLY, self.info, {})
    ...
    def __msg_handle_dispatcher(self, sock, addr):
        ...
        if ((msg.type != Message.TYPE.INIT) and (msg.sender["UNIQUENAME"] not in self.nodes)):
    ...
    def init(self, addr):
        ...
        for num in self.nodes_highest_seq_num:
            if self.nodes_highest_seq_num[num] > highest_num:
                highest_num = self.nodes_highest_seq_num[num]
